...
const DeckManager = ({ toolId, onStudyDeck }) => {
    ...
    if (loading) {
        return <div className="p-8 text-center text-surface-500 animate-pulse font-black uppercase tracking-widest">Loading Library...</div>;
    }
        ...
        <div className="space-y-12 pb-16">
            ...
            <div className="flex justify-between items-center">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-xl font-bold text-sm hover:bg-primary-500 transition-colors shadow-lg shadow-primary-600/20"
                >
            ...
            {showCreateGroup && (
                <form onSubmit={handleCreateGroup} className="bg-surface-900/80 border border-primary-500/30 rounded-2xl p-4 flex gap-3 animate-in fade-in slide-in-from-top-4">
                    <input
                        type="text"
                        autoFocus
                    ...
                    {groups.map((group) => (
                        <div key={group.id} className="space-y-4">
                            ...
                            <div className="flex items-center justify-between pb-2 border-b border-surface-800">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="text-xs font-bold text-surface-400 hover:text-emerald-400 bg-surface-800 hover:bg-emerald-500/10 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                                    >
                                        <Plus size={14} /> Add Deck
                                    </button>
                                    <button
                                        onClick={() => handleDeleteGroup(group.id, group.name)}
                                        className="text-xs font-bold text-surface-500 hover:text-rose-400 bg-surface-800 hover:bg-surface-700 px-3 py-1.5 rounded-lg transition-colors"
                                    >
                                        Delete
                                    </button>
                                </div>
                            ...
                            {showCreateDeckFor === group.id && (
                                <form onSubmit={(e) => handleCreateDeck(e, group.id)} className="bg-surface-900 border border-emerald-500/30 rounded-2xl p-4 flex gap-3 animate-in fade-in zoom-in-95 duration-200">
                                    <input
                                        type="text"
                                        autoFocus
                            ...
                            {!group.decks || group.decks.length === 0 ? (
                                <div className="py-6 px-4 bg-surface-900/30 border border-surface-800/50 border-dashed rounded-2xl text-center">
                                    <p className="text-surface-500 text-sm font-medium">No decks in this subject yet.</p>
                                </div>
                            ) : (
                                        ...
                                        return (
                                            <div key={deck.id} className="bg-surface-900/80 border border-surface-800 hover:border-surface-700 rounded-3xl p-5 transition-all group relative">
                                                <div className="flex justify-between items-start mb-6">
                                                    <div className="w-10 h-10 bg-surface-800 rounded-xl flex items-center justify-center text-surface-400">
                                                        <FolderOpen size={20} />
                                                    </div>
                                                    <button
                                                        onClick={() => handleDeleteDeck(deck.id, deck.name)}
                                                        className="p-2 text-surface-600 hover:text-rose-400 opacity-0 group-hover:opacity-100 transition-opacity"
                                                        title="Delete Deck"
                                                    >
                                                        <Trash2 size={16} />
                                                    </button>
