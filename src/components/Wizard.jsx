...
import { ChevronRight, Target, Clock, Boxes, GraduationCap, CheckCircle2, PenTool, ArrowLeft, Brain, Timer, ClipboardCheck } from 'lucide-react';
    ...
    const steps = [
        {
            ...
            content: (
                <div className="space-y-6">
                    <div className="bg-surface-900/50 border border-surface-800 rounded-[2.5rem] p-8 space-y-4">
                        <div className="w-14 h-14 rounded-2xl bg-primary-500/10 flex items-center justify-center text-primary-400 mb-6">
                            <PenTool size={28} />
                        </div>
                        <label className="text-[10px] font-black text-surface-500 uppercase tracking-[0.3em] block">Tool Name</label>
                        <input
                            type="text"
                            autoFocus
            ...
            content: (
                <div className="space-y-4">
                    <div
                        ...
                        className={`p-6 rounded-3xl border-2 transition-all cursor-pointer flex items-center justify-between group ${data.exam === 'GATE' ? 'border-primary-500 bg-primary-500/10' : 'border-surface-800 bg-surface-900/50 hover:border-surface-700'}`}
                    >
                        <div className="flex items-center gap-4">
                            <div className={`p-3 rounded-2xl ${data.exam === 'GATE' ? 'bg-primary-500 text-white' : 'bg-surface-800 text-surface-400 group-hover:text-white'}`}>
                                <GraduationCap size={24} />
                            </div>
                            <div>
                                <h4 className="font-black uppercase tracking-tight text-lg">GATE 2027</h4>
                                <p className="text-xs text-surface-500 font-bold uppercase tracking-widest">Graduate Aptitude Test in Engineering</p>
                            </div>
                        </div>
                        {data.exam === 'GATE' && <CheckCircle2 className="text-primary-400" />}
                    </div>

                    <div className="p-6 rounded-3xl border border-surface-800/50 bg-surface-900/20 opacity-40 cursor-not-allowed flex items-center gap-4">
                        <div className="p-3 bg-surface-800 rounded-2xl text-surface-600"><Target size={24} /></div>
                        <div>
                            <h4 className="font-black uppercase tracking-tight text-lg">ESE / IES</h4>
                            <p className="text-[10px] text-surface-600 font-black uppercase tracking-widest italic">Coming Soon</p>
                        </div>
                    </div>
                </div>
            )
        },
        {
            ...
            content: (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    >
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${data.mode === 'module' ? 'bg-primary-500 text-white' : 'bg-surface-800 text-surface-500 group-hover:text-white'}`}>
                            <Boxes size={32} />
                        </div>
                        <div>
        ...
        <div className="fixed inset-0 z-[200] bg-base flex items-center justify-center p-4">
            <div className="max-w-xl w-full">
                ...
                <div className="flex justify-center gap-2 mb-12">
                    {steps.map(s => (
                        <div key={s.id} className={`h-1.5 rounded-full transition-all duration-500 ${s.id === step ? 'w-12 bg-primary-500' : s.id < step ? 'w-6 bg-primary-500/40' : 'w-4 bg-surface-800'}`} />
                    ))}
                </div>
                ...
                <div className="flex gap-3">
                    ...
                    <button
                        ...
                        disabled={!canProceed}
                        className="flex-1 bg-white text-black py-5 rounded-2xl font-black text-lg uppercase tracking-widest hover:bg-primary-500 hover:text-white transition-all shadow-2xl shadow-primary-600/10 flex items-center justify-center gap-3 disabled:opacity-30 disabled:cursor-not-allowed"
                    >
