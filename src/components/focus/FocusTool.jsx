    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-surface-800 pb-8">
                ...
                <div className="bg-primary-500/10 text-primary-400 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-surface-900/50 border border-surface-800 p-8 rounded-[2rem] relative overflow-hidden group">
                ...
                <div className="bg-surface-900/50 border border-surface-800 p-8 rounded-[2rem] relative overflow-hidden group md:col-span-1 lg:col-span-2">
                    <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
                        <CalendarDays size={100} />
                    </div>
                    <div className="relative z-10">
                        <p className="text-[10px] font-black text-surface-500 uppercase tracking-widest mb-2">Total Focus Time All-Time</p>
                        <p className="text-5xl font-black text-primary-400 tracking-tighter">{formatMinsToHours(stats.totalTime)}</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                {recentSessions.length === 0 ? (
                    <div className="bg-surface-900/50 border border-surface-800 border-dashed rounded-[3rem] p-16 text-center">
                        ...
                        <h4 className="text-surface-400 font-bold uppercase tracking-widest text-lg mb-2">No focus sessions yet</h4>
                        <p className="text-surface-500 text-sm max-w-sm mx-auto">Click "Start Focus Mode" on your dashboard to log your first intense study session.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {recentSessions.map(session => (
                            <div key={session.id} className="bg-surface-900/50 border border-surface-800 rounded-3xl p-6 flex flex-col justify-between hover:border-primary-500/30 transition-colors">
                                ...
                                {session.notes && (
                                    <div className="bg-surface-950/50 rounded-xl p-4 mb-6">
                                        <p className="text-surface-400 text-sm italic">"{session.notes}"</p>
                                    </div>
                                )}

                                <div className="mt-auto">
                                    {session.linked_topic_id ? (
                                        <div className="inline-flex items-center gap-2 px-3 py-2 bg-surface-800 text-surface-400 rounded-lg text-[10px] font-black uppercase tracking-wide w-full truncate">
                                            <Hash size={14} className="text-primary-400 shrink-0" />
                                            <span className="truncate">{session.subject_name} / {session.topic_name}</span>
                                        </div>
                                    ) : (
                                        <div className="inline-flex items-center gap-2 px-3 py-2 bg-surface-900 border border-surface-800 text-surface-500 rounded-lg text-[10px] font-black uppercase tracking-wide w-full">
