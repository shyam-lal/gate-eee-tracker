...
const StudyMaterials = ({ examId, examName, syllabus, onBack }) => {
    ...
    return (
        <div className="min-h-screen bg-base text-surface-400">
            {/* Background */}
            <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-primary-600/10 rounded-full blur-[120px] pointer-events-none" />
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 relative z-10">
                ...
                <div className="mb-6 space-y-3">
                    <div className="flex gap-3">
                        <div className="flex-1 flex items-center bg-surface-900/60 border border-surface-800 rounded-2xl px-4 py-3 gap-3 focus-within:border-primary-500/50 transition-colors">
                        ...
                        <button
                            ...
                            className={`p-3 border rounded-2xl transition-all ${
                                showFilters || filterSubject || filterType
                                    ...
                                    : 'bg-surface-900/60 border-surface-800 text-surface-500 hover:text-white'
                            }`}
                        >
                            <Filter size={16} />
                        </button>
                ...
                {loading ? (
                    <div className="text-center py-20 text-surface-600">Loading materials...</div>
                ) : Object.keys(grouped).length === 0 ? (
                    <div className="text-center py-20">
                        <BookOpen size={48} className="mx-auto mb-4 text-surface-700 opacity-30" />
                        <h3 className="text-lg font-bold text-surface-500 mb-1">No Materials Found</h3>
                        ...
                        {Object.entries(grouped).map(([subjectName, items]) => (
                            <div key={subjectName}>
                                <div className="flex items-center gap-3 mb-4 px-1">
                                    ...
                                    <h3 className="text-xs font-black text-surface-400 uppercase tracking-widest">{subjectName}</h3>
                                    <span className="text-[10px] text-surface-600 font-bold">{items.length}</span>
                                </div>
                                    ...
                                    {items.map(mat => {
                                        ...
                                        return (
                                            <a
                                                ...
                                                className="group relative bg-[#0b1120] border border-surface-800 rounded-3xl p-6 overflow-hidden hover:border-primary-500/50 transition-all duration-300 flex flex-col cursor-pointer hover:shadow-2xl hover:shadow-primary-500/10 hover:-translate-y-1 block h-full"
                                                onClick={(e) => {
                                                    if (!mat.file_url) {
                                                        e.preventDefault();
                                            ...
                                            >
                                                ...
                                                <div className="relative z-10 flex flex-col h-full">
                                                    <div className="flex justify-between items-start mb-6">
                                                        ...
                                                        <span className="text-[10px] bg-surface-800/80 backdrop-blur-sm text-surface-400 px-3 py-1 rounded-full font-bold uppercase tracking-widest border border-surface-700/50">
                                                            {config.label}
                                                        </span>
                                                    </div>
                                                    ...
                                                    {mat.topic_name && (
                                                        <p className="text-xs text-surface-500 font-bold mb-4 line-clamp-1">
                                                            {mat.topic_name}
                                                        </p>
                                                    )}

                                                    <div className="mt-auto pt-4 flex items-center justify-between border-t border-surface-800/50">
                                                        <div className="flex items-center gap-2 text-[10px] text-surface-500 font-bold uppercase tracking-widest">
                                                            {mat.file_size_bytes ? (
                                                        ...
                                                        <div className="w-8 h-8 rounded-full bg-surface-800 flex items-center justify-center text-surface-400 group-hover:bg-primary-500 group-hover:text-white transition-all transform group-hover:scale-110">
            ...
            {selectedMaterial && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6 animate-in fade-in">
                    <div className="bg-surface-950 border border-surface-700 w-full max-w-3xl max-h-[90vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                        <div className="bg-surface-900 px-6 py-5 border-b border-surface-800 flex justify-between items-start">
                            <div className="flex-1 pr-4">
                                <div className="flex items-center gap-2 mb-2">
                                    <span className="text-[10px] bg-primary-500/20 text-primary-400 px-2.5 py-1 rounded-full font-bold uppercase tracking-widest border border-primary-500/20">
                        ...
                        <div className="p-6 overflow-y-auto custom-scrollbar">
                            {selectedMaterial.content ? (
                                <div className="prose prose-invert prose-slate max-w-none">
                                    <pre className="whitespace-pre-wrap text-[15px] text-surface-400 leading-relaxed font-sans bg-transparent p-0 m-0">
                                        {selectedMaterial.content}
                                    </pre>
                                </div>
                            ) : (
                                <div className="text-center py-12">
                                    <p className="text-surface-500 font-bold">No content available.</p>
                                </div>
                            )}
                        </div>
                    </div>
