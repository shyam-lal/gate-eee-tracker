...
const Social = ({ currentUser, onBack }) => {
    ...
    const renderMedal = (type) => {
        switch (type) {
            ...
            case 'award': return <Trophy className="text-primary-400" size={24} />;
            case 'clock': return <Medal className="text-emerald-400" size={24} />;
            case 'box': return <Award className="text-purple-400" size={24} />;
            default: return <Award className="text-surface-400" size={24} />;
        }
    };
        ...
        return (
            <div className="min-h-screen bg-base text-surface-400 p-6 animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="max-w-4xl mx-auto space-y-12">
                    <div className="flex flex-col md:flex-row items-center gap-8 bg-surface-900/40 border border-surface-800 p-10 rounded-[3rem]">
                        <div className="w-32 h-32 bg-gradient-to-br from-primary-500 to-secondary-600 rounded-[2.5rem] flex items-center justify-center text-white text-5xl font-black shadow-2xl shadow-primary-600/20">
                        ...
                        <div className="text-center md:text-left flex-1">
                            ...
                            <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                                ...
                                <button
                                    ...
                                    className={`px-6 py-2 rounded-xl font-black uppercase text-xs tracking-widest transition-all ${selectedUserProfile.isFollowing ? 'bg-surface-800 text-surface-400 hover:bg-rose-500/10 hover:text-rose-500' : 'bg-primary-600 text-white hover:bg-primary-500 shadow-lg shadow-primary-600/20'}`}
                                >
                        ...
                        <div className="bg-surface-900/30 border border-surface-800 rounded-[2.5rem] p-8">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                {selectedUserProfile.achievements?.length > 0 ? selectedUserProfile.achievements.map((ach, idx) => (
                                    <div key={idx} className="flex flex-col items-center gap-2 p-4 bg-surface-950/50 rounded-2xl border border-surface-800/50 group">
                                        ...
                                        <span className="text-[8px] font-black text-surface-500 uppercase text-center leading-tight">{ach.name}</span>
                                    </div>
                                )) : (
                                    <p className="col-span-3 text-center text-surface-600 font-bold uppercase text-[10px] tracking-widest py-8">No medals collected yet</p>
                                )}
                            </div>
                        </div>
        ...
        <div className="min-h-screen bg-base text-surface-400 flex flex-col pt-12">
            <nav className="max-w-6xl mx-auto w-full px-6 flex justify-between items-center mb-12">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-black uppercase tracking-tighter">Social <span className="text-primary-500">Terminal</span></h1>
                </div>
            </nav>
            ...
            <main className="max-w-6xl mx-auto w-full px-6 flex-1 flex flex-col lg:flex-row gap-12">
                ...
                <aside className="lg:w-80 shrink-0 space-y-8">
                    <div className="relative group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-surface-500 group-focus-within:text-primary-500 transition-colors" size={18} />
                        <input
                            type="text"
                            placeholder="Search Engineers..."
                            className="w-full bg-surface-900/50 border border-surface-800 rounded-2xl py-4 pl-12 pr-4 text-sm font-bold placeholder-surface-600 focus:border-primary-500 outline-none transition-all"
                            value={searchQuery}
                    ...
                    <nav className="flex flex-col gap-2">
                        <button
                            ...
                            className={`flex items-center justify-between p-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] transition-all ${activeTab === 'friends' ? 'bg-primary-600 text-white shadow-lg shadow-primary-600/20' : 'bg-surface-900/30 text-surface-500 hover:bg-surface-800/50'}`}
                        >
                        ...
                        <button
                            ...
                            className={`flex items-center justify-between p-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] transition-all ${activeTab === 'achievements' ? 'bg-primary-600 text-white shadow-lg shadow-primary-600/20' : 'bg-surface-900/30 text-surface-500 hover:bg-surface-800/50'}`}
                        >
                    ...
                    {searchQuery.length >= 2 && (
                        <div className="bg-surface-900/30 border border-surface-800 rounded-[2rem] overflow-hidden animate-in slide-in-from-top-4 duration-300">
                            <div className="px-4 py-3 border-b border-surface-800 bg-surface-800/20">
                                <span className="text-[9px] font-black text-surface-500 uppercase tracking-widest">Search Results</span>
                            </div>
                            <div className="max-h-[300px] overflow-y-auto">
                                {searchResults.length > 0 ? searchResults.map(user => (
                                    <div key={user.id} className="flex items-center justify-between p-4 hover:bg-surface-800/50 transition-all border-b border-surface-800/50 last:border-0">
                                ...
                                )) : <p className="p-4 text-center text-[10px] font-bold text-surface-600">No engineers found</p>}
                            </div>
                        </div>
                    )}
                </aside>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {following.length > 0 ? following.map(user => (
                                    <div key={user.id} onClick={() => viewProfile(user.id)} className="flex items-center justify-between bg-surface-900/40 p-5 rounded-3xl border border-surface-800 hover:border-primary-500/30 transition-all cursor-pointer group">
                                        ...
                                        <ChevronRight size={18} className="text-surface-700 group-hover:text-primary-500 transition-all" />
                                    </div>
                                )) : (
                                    <div className="col-span-full py-20 text-center space-y-4">
                                        <Users size={48} className="text-slate-800 mx-auto" />
                                        <p className="text-surface-500 font-bold uppercase text-xs tracking-[0.2em]">Your fleet is empty. Search for engineers to follow.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                                {achievements.length > 0 ? achievements.map((ach, idx) => (
                                    <div key={idx} className="flex flex-col items-center gap-4 p-8 bg-surface-900/60 rounded-[2rem] border border-surface-800 shadow-xl shadow-black/20 group hover:scale-105 transition-all">
                                        <div className="w-20 h-20 bg-primary-500/10 rounded-full flex items-center justify-center text-primary-400 border border-primary-500/20 drop-shadow-[0_0_15px_rgba(99,102,241,0.2)]">
                                            {renderMedal(ach.icon_name)}
                                        </div>
                                    ...
                                    <div className="col-span-full py-20 text-center space-y-4">
                                        ...
                                        <p className="text-surface-500 font-bold uppercase text-xs tracking-[0.2em]">No achievements earned yet. Start sync sessions to earn medals.</p>
                                    </div>
                                )}
                            </div>
                        </div>
