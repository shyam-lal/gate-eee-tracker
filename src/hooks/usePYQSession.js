...
/**
 * usePYQSession — reusable hook for all test-taking logic for PYQ Papers.
 * Models exactly after useTestSession.js but uses pyqApi.
 *
 * @param {Object} config
 * @param {Object} config.set         — the question set (paper) { id, title, topics, time_per_question, ... }
 ...
 */
export default function usePYQSession({ set, attempt, mode = 'exam', toolId, onComplete, onExit }) {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const data = await pyqApi.getPaper(set.id);
    ...
    const saveCurrentAnswer = async () => {
        ...
        if (answer !== undefined && answer !== null) {
            try {
                await pyqApi.saveAnswer(attempt.id, qId, answer, spent);
    ...
    const pause = async () => {
        ...
        await saveCurrentAnswer();
        try {
            await pyqApi.pauseAttempt(attempt.id, currentIdx, totalTime);
        } catch (err) { console.error(err); }
    };
    ...
    const handleSubmit = async () => {
        ...
        await saveCurrentAnswer();
        try {
            const result = await pyqApi.completeAttempt(attempt.id, toolId);
