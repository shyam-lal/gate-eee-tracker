...
const adminUsersController = {
    ...
    getAllUsers: async (req, res) => {
        try {
            ...
            let query = `
                SELECT 
                    u.id, u.username, u.email, u.role, u.created_at, u.ai_generation_mode, u.credits
                FROM users u
                WHERE 1=1
            `;
            const queryParams = [];
    ...
    getUserDetails: async (req, res) => {
        try {
            ...
            const userRes = await pool.query(
                `SELECT id, username, email, role, created_at, ai_generation_mode, credits FROM users WHERE id = $1`,
                [id]
            );
