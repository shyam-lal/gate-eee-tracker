...
const fullMigrate = async () => {
    try {
        console.log('Starting full database migration from scratch...');

        // Ordered list of ALL migrations, starting with init.sql
        const migrations = [
            'init.sql',
            'migration_v2.sql',
            ...
            'migration_social.sql' // Adding social migration at the end just in case
        ];
        ...
        console.log('✅ All migrations executed successfully! Fresh database schema is ready.');
    } catch (err) {
        console.error('❌ Full Migration failed:', err);
    } finally {
        await pool.end();
    }
};
